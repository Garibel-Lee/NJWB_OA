...
public class HolidayMapper implements RowMapper {
    ...
    public List<Holiday> mapperList(ResultSet rs) throws SQLException {
        logger.debug("获得t_holiday表中数据，这是t_holiday表与HolidayMapper类的映射");
        ...
        while (rs.next()) {
            ...
            holiday.setHolidayNo(Integer.valueOf(rs.getString("t_holiday_no")));
            holiday.setHolidayUser(rs.getString("t_holiday_user"));
            holiday.setHolidayType(Integer.valueOf(rs.getString("t_holiday_type")));
            holiday.setHolidayBz(rs.getString("t_holiday_bz"));
            holiday.setHolidayStatus(rs.getString("t_holiday_status"));
            holiday.setCreateTime(LocalDateTime.parse(rs.getString("t_create_time"),DateTimeFormatter.ofPattern("yyyy-MM-dd ")));
            try {
